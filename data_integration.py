...
class DataExtraction:
    def __init__(self,doc_folder):
        print('in dataExtraction in data_integration')
        # logging.basicConfig(
        #     format="%(asctime)s %(levelname)-8s [%(filename)s:%(lineno)d] %(message)s",
        #     datefmt="%d-%m-%Y %H:%M:%S",
        #     level=logging.INFO,
        #     filename="dextract.log",
        # )
    ...
    def get_extracted_data(self):  
        try:      
            ...
            for file in os.listdir(self.doc_folder):
                if file.endswith('pptx'):                
                    ...
                    try:           
                        ...
                        self.logger.error(f"get_extracted_data: {err}")
                    ## need to uncomment before final release
                    try:
            ...
            self.logger.error(f"get_data_extracted: {err}")    
